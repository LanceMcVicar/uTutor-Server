<?php
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$requiredParameters = array("tutorEmail","tuteeEmail","startAppDateTime","endAppDateTime", "status");
		$params=array();
		...
		$sql = "UPDATE appointments SET is_accepted=? 
				WHERE tutorEmail = ? AND tuteeEmail=? AND startAppDateTime = ? AND endAppDateTime=?";
		if(!$stmt = $conn->prepare($sql)){	
			$err = array('error' => -3 , 'errormessage' => 'Query Failed to Prepare');
			echo json_encode($err);
			die;
		}		
		if(!$stmt -> bind_param("sssss",$_POST["status"],$_POST["tutorEmail"],$_POST["tuteeEmail"],$_POST["startAppDateTime"],$_POST["endAppDateTime"])){	
